...
export default function ArticoloSingolo() {
  ...
  const correlati = articoli.filter((a) => a.categoriaKey === articolo.categoriaKey && a.slug !== articolo.slug).slice(0, 3);
  const popolari = articoli.filter((a) => a.slug !== articolo.slug).slice(0, 4);
  const tr = t[locale].article;
    ...
    <article>
      ...
      <div className="container py-14 grid lg:grid-cols-[1fr_300px] gap-14">
        <div className="prose-editorial max-w-3xl">
          <p className="text-xl md:text-2xl font-display text-foreground/80 leading-relaxed mb-10">{articolo.intro}</p>
          ...
          <div className="mt-12 pt-10 border-t border-border flex items-start gap-5">
            ...
            <div>
              <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1 font-semibold">{tr.writtenBy}</div>
              <Link to={lp(`/autore/${autore.slug}`, locale)} className="font-display text-xl font-semibold hover:text-primary">
                {autore.nome}
              </Link>
        ...
        <aside>
          <h3 className="font-display text-lg font-semibold mb-4 pb-3 border-b border-border">{tr.aboutAuthor}</h3>
          <Link to={lp(`/autore/${autore.slug}`, locale)} className="flex items-center gap-3 mb-8 group">
          ...
          <h4 className="font-display text-lg font-semibold mb-4 pb-3 border-b border-border">{tr.categories}</h4>
          <div className="flex flex-wrap gap-2 mb-8">
            <Link to={lp(`/articoli/${categorySlug(articolo.categoriaKey, locale)}`, locale)}>
              <CategoryBadge categoria={articolo.categoria} categoriaKey={articolo.categoriaKey} />
            </Link>
          </div>

          <h4 className="font-display text-lg font-semibold mb-4 pb-3 border-b border-border">{tr.popular}</h4>
          <ul className="space-y-5">
            {popolari.map((a) => (
              <li key={a.slug}>
                <Link to={lp(`/articolo/${a.slug}`, locale)} className="block group">
      ...
      <section className="bg-paper py-16">
        <div className="container">
          <h2 className="font-display text-3xl font-bold mb-10">{tr.related}</h2>
